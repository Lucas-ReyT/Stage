...
ui <- fluidPage(
  fileInput("file", "Sélectionnez le fichier Excel"),
  actionButton("btn_process", "Traiter les données"),
...
server <- function(input, output) {
  ...
  observeEvent(input$btn_process, {
    req(input$file)
    # On charge chaque fiche dans une variable
    ...
    Tree_app$Format$Metadata$Experiment_name$df <- LongueurDF(Metadata, "Experiment_name", 0)
